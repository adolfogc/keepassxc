    ...
    , m_buttonBox(new QDialogButtonBox(QDialogButtonBox::Close | QDialogButtonBox::Ok))
{
    ...
    const bool autoClose = config()->get("security/AutoCloseTotpExportSettingsDialog", true).toBool();
    if (autoClose) {
        m_secTillClose = config()->get("security/AutoCloseTotpExportSettingsDialogTimeout", 45).toInt();
        m_countDown->setText(tr("Closing in %n seconds.", "", m_secTillClose));
        m_timer->start(1000);
    } else {
        m_countDown->setText("");
    }
